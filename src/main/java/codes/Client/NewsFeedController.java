...
public class NewsFeedController {
    ...
    private HBox createUserRow(ClientInfo user, String type) {
        ...
        img.setClip(new Circle(16,16,16));
    ...
    private void onPendingRequestsClicked() {
            ...
            for (ClientInfo requester : pending) {
                ...
                String buttonStyle =
                        "-fx-background-color: #e8f0fe;" +
                                ...
                                "-fx-border-radius: 10;" +
                                "-fx-padding: 6 14;" +
                                "-fx-border-color: #c6dafc;" +
                                "-fx-border-width: 1;" +
                                "-fx-cursor: hand;";

                accept.setStyle(buttonStyle);
                reject.setStyle(buttonStyle);

                String hoverStyle =
                        "-fx-background-color: #d2e3fc;" +  // slightly darker on hover
                                "-fx-text-fill: #1a73e8;" +
                                "-fx-font-weight: bold;" +
                                "-fx-background-radius: 10;" +
                                "-fx-border-radius: 10;" +
                ...
                accept.setOnMouseEntered(e -> accept.setStyle(hoverStyle));
                accept.setOnMouseExited(e -> accept.setStyle(buttonStyle));

                reject.setOnMouseEntered(e -> reject.setStyle(hoverStyle));
                reject.setOnMouseExited(e -> reject.setStyle(buttonStyle));
        ...
        scrollPane.viewportBoundsProperty().addListener((obs, oldBounds, newBounds) -> {
            container.setMinHeight(newBounds.getHeight());
        });
    ...
    public void loadCurrentFriends() {
        ...
        allCurrentFriends = client.getFriendList();
