...
public class Client {
    ...
    public Client(String ipAddress) {
        ...
        try {
            this.serverSocket = new Socket(ipAddress, 4349);
            ...
            while (true) {
                ...
                if (fromServer instanceof Boolean b) {
                    ...
                    if (!active) {
                        ...
                        Platform.runLater(() -> {
                            profilePicture = ImageCropper.cropImageToSquare(new Image(new ByteArrayInputStream(profilePictureByte)));
                        });
                    } else {
                        callerInfo = info;
                    }
                    ...
                    this.receiverId = string.substring("connect_to:".length());

                    try {
                        this.chatSocket = new Socket(ipAddress, 4350);
                        ...
                        chatOutput.writeObject(id);
                        chatOutput.writeObject(receiverId);
                    } catch (IOException e) {
                        throw new RuntimeException(e);
                    }
                    ...
                    if (notification.get(sender) != null) notification.remove(sender);
                    notification.put(sender, new Pair(type, status));
                    if (status.equals("unseen")) newNotification = true;
                    if (type.equals("message")) System.out.println(sender + " sent a " + notification.get(sender));
                }
                    ...
                    System.out.println("call " + response + " in " + firstName);
                }
                    ...
                    if (!map.isEmpty() && map.keySet().iterator().next() instanceof String) {
                        ...
                        if (fetchFriendStatusLatch != null) { fetchFriendStatusLatch.countDown(); fetchFriendStatusLatch = null; }
                    }
                }
                    ...
                    if (!list.isEmpty() && list.get(0) instanceof ClientInfo) {
                        ...
                        if (friendsSentTheMessage) {
                            ...
                            friendsSentTheMessage = false;
                            if (clientListLatch != null) { clientListLatch.countDown(); clientListLatch = null; }
                        } else if (latch != null) {
                            if (gettingRequests) {
    ...
    public void clientIsConnectedToNewsFeed() { connectedToNewsFeed = true; }

    public boolean isConnectedToNewsFeed() { return connectedToNewsFeed; }
    ...
    public void disconnectFromChatServer() {
        try {
            ...
            System.out.println(this.id + ": Disconnected from Chat Server");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public boolean getChatStatus() { return chatStatus; }
    public String getReceiverIPAddress() { return receiverIPAddress; }
    public Stage getStage() { return stage; }
    public IntroPage getIntroPage() { return introPage; }
    public LoginPage getLoginPage() { return loginPage; }
    public AboutUsPage getAboutUsPage() { return aboutUsPage; }
    public SignupPage getSignupPage() { return signupPage; }
    public ProfilePicturePage getProfilePicturePage() { return profilePicturePage; }
    public ForgotPasswordPage getForgotPasswordPage() { return forgotPasswordPage; }
    public HomePage getHomePage() { return homePage; }
    public Inbox getInbox() { return inbox; }
    public NewsFeed getNewsFeed() { return newsFeed; }
    public ObjectOutputStream getServerOutput() { return serverOutput; }
    public ObjectInputStream getServerInput() { return serverInput; }
    public ObjectOutputStream getChatOutput() { return chatOutput; }
    public ObjectInputStream getChatInput() { return chatInput; }
    public ObjectOutputStream getFeedOutput() { return feedOutput; }
    public ObjectInputStream getFeedInput() { return feedInput; }
    public Vector<ClientInfo> getClients() { return clients; }
    ...
    @Override
    public String toString() { return id + "," + password + "," + firstName + "," + lastName; }
                        ...
                        if (newsFeedController != null) {
                            if (feedUpdate instanceof String stringUpdate) {
                                if (stringUpdate.startsWith("REACTION|")) {
                                    ...
                                    newsFeedController.addPostToFeed(stringUpdate);
                                }
                            } else if (feedUpdate instanceof PostPacket packet) {
                                newsFeedController.addPostToFeed(packet);
    ...
    public String getMyId() { return this.id; }

    public Vector<ClientInfo> getAllClients() { return this.clients; }
