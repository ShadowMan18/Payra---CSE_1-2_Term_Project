...
public class InboxController {
    ...
    @FXML
    public Group receiverInfo;
    @FXML
    public HBox activeStatus;
    @FXML
    public Group callButtons;
    ...
    public void setInboxController(Client client, Stage stage) {
        ...
        receiverInfo.setOpacity(0);
        activeStatus.setOpacity(0);
        callButtons.setDisable(true);
        callButtons.setOpacity(0);
    ...
    public void startChat(String id) {
        ...
        receiverInfo.setOpacity(1);
        if (!receiverId.equals(client.getId())) {
            callButtons.setOpacity(1);
            callButtons.setDisable(false);
        }
        else {
            callButtons.setOpacity(0);
            callButtons.setDisable(true);
        ...
        for (ClientInfo clientInfo : client.getClients()) {
            if (clientInfo.getId().equals(receiverId)) {
                ...
                receiverProfilePicture = ImageCropper.cropImageToSquare(new Image(new ByteArrayInputStream(clientInfo.getProfilePicture())));
            }
        }
        ...
        new Thread(() -> {
            while (true) {
                Object fromChatServer;

                try {
                    fromChatServer = client.getChatInput().readObject();
                } catch (IOException | ClassNotFoundException e) {
                    break;
                }

                if (fromChatServer instanceof MessagePacket) {
                    MessagePacket messagePacket = (MessagePacket) fromChatServer;
                    String message = messagePacket.getMessage();
                    String sender = messagePacket.getSender();
                    String filename = messagePacket.getFilename();
                    byte[] fileBytes = messagePacket.getFiledata();
                    LocalDateTime datetime = messagePacket.getDatatime();
                    String dateFormat = "";
                ...
                else if (fromChatServer instanceof String){
                    String status = (String) fromChatServer;
                    if (status.equals("active") && !receiverId.equals(client.getId())) {
                        activeStatus.setOpacity(1);
                    }
                    else {
                        activeStatus.setOpacity(0);
                    }
...
//        }
    ...
    public void displayUsers(String prefix) {
        ...
        for (ClientInfo clientInfo : displayableUsers) {
            Image profilePicture = ImageCropper.cropImageToSquare(new Image(new ByteArrayInputStream(clientInfo.getProfilePicture())));
            ImageView profilePictureView = new ImageView(profilePicture);
